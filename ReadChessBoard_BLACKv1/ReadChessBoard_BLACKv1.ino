...
//char * squaresWB[] = {"G8","G7","G6","G5","G4","G3","G2","G1","H8","H7","H6","H5","H4","H3","H2","H1","E8","E7","E6","E5","E4","E3","E2","E1","F8","F7","F6","F5","F4","F3","F2","F1","C8","C7","C6","C5","C4","C3","C2","C1","D8","D7","D6","D5","D4","D3","D2","D1","A8","A7","A6","A5","A4","A3","A2","A1","B8","B7","B6","B5","B4","B3","B2","B1"};
...
//Buttons
const int turnCompleteButton = 45;
byte turnCompleteVal;
const int newGameButton = 47;
byte newGameVal;
const int checkButton = 49;
byte checkVal;


//Leds
const int newGameLED = 43;
const int checkLED = 39;
const int turnCompleteLED = 41;
...
//String movePieceIn = "H7H500;
Serial.println(movePieceIn);
 ...
 Serial.println("Send to Uno");
  ...
  {
    delay(1000);
    Serial.println("waiting for uno");
  }
...
void readBoard(){
  ...
  while(turnCompleteVal == LOW) 
  {
    ...
    if (takenSensorVal == 0 && mode == 0)
    {
      for (int i=0; i<16; i++)
      {
        ...
        boardStateTAKEN[i]=megaDigitalValuesTAKEN[i];
        boardStateTAKEN[i+16]=mux3DigitalValuesTAKEN[i];
        boardStateTAKEN[i+32]=mux2DigitalValuesTAKEN[i];
        boardStateTAKEN[i+48]=mux1DigitalValuesTAKEN[i];
      }
  ...
  for (int i=0; i<64; i++)
  {
    boardStateNEW[i] = boardStateNEWTmp[i];
  }
